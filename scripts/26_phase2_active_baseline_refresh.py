"""Refresh Phase 2 statistical baseline on the active analytical population.
...
Inputs:
  --active-dir   data/processed/phase2-active (primary)
  --population   bgg_research_population.parquet for game identity
  --phase2-dir   data/processed/phase2 (historical reference for comparison)
  --out-dir      active output dir (default: active-dir)

Outputs under out-dir (gitignored):
  user_severity_active.parquet
  game_adjusted_means_active.parquet
  active_baseline_refresh.json   full summary + validation + comparison
  active_band_cells.parquet      game x band aggregates (for reuse)
  within_game_diffs_active_*.parquet  paired diff tables
  gap_cells_active.parquet       low/high cells for decomposition
...
def main():
    ...
    ap.add_argument("--active-dir", type=Path, default=None,
                    help="Active extracts dir (default: data/processed/phase2-active)")
    ...
    ap.add_argument("--out-dir", type=Path, default=None,
                    help="Output dir for active baseline artefacts (default: active-dir)")
    ...
    out_dir = args.out_dir or active_dir
    out_dir.mkdir(parents=True, exist_ok=True)

    # Fail closed if active extracts not present (sequencing gate)
    ro_active_path = active_dir / "rating_observations_active.parquet"
    users_active_path = active_dir / "users_active.parquet"
    if not ro_active_path.exists():
        raise FileNotFoundError(f"Active extracts missing: {ro_active_path} — run scripts/24_build_active_phase2_extracts.py first")
    if not users_active_path.exists():
        raise FileNotFoundError(f"Active extracts missing: {users_active_path}")
    ...
    # ------------------------------------------------------------------
    print(f"Active dir: {active_dir}", flush=True)
    ...
    assert pop_count == 16627, f"Expected 16627 games in population, got {pop_count}"
    ...
    # Also validate that active observations count matches extract_counts expectation (24,509,788)
    validation["expected_active_observations"] = 24509788
    validation["expected_active_users"] = 288730
    validation["delta_obs_vs_expected"] = int(active_obs - 24509788)
    validation["delta_users_vs_expected"] = int(active_users - 288730)
    assert abs(active_obs - 24509788) < 10, f"active obs mismatch: {active_obs} vs 24509788"
    assert abs(active_users - 288730) < 10, f"active users mismatch: {active_users} vs 288730"
    ...
    print(f"  validations passed: {active_obs:,} obs, {active_users:,} users, {active_games:,} games", flush=True)
    ...
    cells_path = out_dir / "active_band_cells.parquet"
    con.execute(f"""
        COPY (
            SELECT game_id,
                   volume_band,
    ...
    for name, (low_bands, high_bands) in CONTRASTS.items():
        ...
        pq.write_table(pa.Table.from_pandas(df, preserve_index=False),
                       out_dir / f"within_game_diffs_active_{name}.parquet", compression="zstd")
        del df
    ...
    sev_path = out_dir / "user_severity_active.parquet"
    gm_path = out_dir / "game_adjusted_means_active.parquet"
    ...
    # Persist gap cells
    gap_cells_path = out_dir / "gap_cells_active.parquet"
    ...
    # Write main JSON
    out_json = out_dir / "active_baseline_refresh.json"
    out_json.write_text(json.dumps(summary, indent=2, default=str) + "\n", encoding="utf-8")
    print(f"Wrote {out_json}", flush=True)

    # Also write a lightweight validation JSON next to artefacts (spec)
    val_path = out_dir / "active_baseline_validation.json"
    val_payload = {
        ...
        "artefacts": {
            "user_severity_active": str(sev_path),
            "game_adjusted_means_active": str(gm_path),
            "active_band_cells": str(cells_path),
            "gap_cells_active": str(gap_cells_path),
        }
    }

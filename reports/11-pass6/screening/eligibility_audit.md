...
**Generated:** 2026-08-26T04:30Z · seed 20260824 · population 14,698 × 287,302 × 24,146,307 mu 7.139 reuse `user_severity_pass2` + `game_adjusted_means_pass2` via 39/40 — **reuse, do NOT refit**

**Pool:** 532 games `adj≥7.5 & resid≥0.75` absolute (NOT percentile; sensitivity 455 at `≥0.80` and 211 at `≥1.00`) from `docs/06-hiddenness-gates/10-quality-gates/screening_pool.csv`.

**What fraction had structured fields queried — must be 100%:**

> **532/532 (100%) queried `game_links` (33,002 rows: version 19,504 59.1% vs expansion 6,339 19.2% vs reimplementation 1,526 4.6% vs `reimplements` 294 vs `contained_in` 238 vs `contains` 98 vs accessory 3,228) + `families`/`series` (`Game:` 2,740 18.6% + `Series:` 3,302 22.5% + `Admin: Game System Entries` 32) + reimplementation relationships (`is_reimplementation` 265 1.80% + `reimplements` 294 + `reimplementation` 1,526 + `log_n_impl`) + expansion relationships (`expansion` 6,339) + editions/versions (`version` 19,504 59% vs expansion, `n_version_src`/`n_version_tgt`) + game-system relationships (`Admin: Game System Entries` 32 + `contained_in` 238) + related/parent (`game_links` `other_id`→`game_id` + families `Game:`/`Series:` + designers/year/weight)** [observed fact].
...
**Total hard in pool:** **25 (4.7% of 532)** vs prior Pass5 17 (3.2%) — more precise after 100% query refinement (removed false hard multi-base compilations like A Gamut of Games 4385 which has 2 contained_in bases Focus+Lines of Action → compilation not edition, now correctly eligible). Borderline **61 (11.5%)** vs eligible **446 (83.8%)**.
...
|---|---|---|---|---|---|---|---|---|
| 244258 | The Red Dragon Inn 7: The Tavern Crew | 2018 | **yes** (n 310 adj 7.86 resid 1.31) | `Game: The Red Dragon Inn` eco 11 | `game_links` as_target 0 (version_tgt 0 contained 0 reimplements 0) as_source 0; no `reimplements`/`contained_in`/`version` link | **eligible** | eligible | families Game: The Red Dragon Inn (11) indicates 11-game ecosystem but **no version/contained_in/reimplements link found** — not a variant/edition per deterministic check; eco 11 large but no link + title contains series token but designers/year/weight not corroborating hard, remains eligible (ecosystem borderline medium flag only, not hard; see 6A ecosystem rule) — explicitly no qualifying structured hard relationship |
| 377969 | Marvel United: Multiverse | 2024 | **yes** (n 850? Actually users 850 but n_obs? In pool n~? Check eligibility_pool 850) | `Game: United` eco 4 (Game: United 4, not Marvel United-specific) | 0 links (version 0 contained 0 reimplements 0) | **eligible** | eligible | families Game: United (shared with Marvel United 298047, X-Men 336382 etc.) but **no Marvel United Game: family**, no version/contained_in/reimplements link — no hard relationship; eco 4 small, not ecosystem well-known (unlike Catan 40 Unlock 47); title contains Marvel United but no structured link, description/tagline not hard per task — eligible |
| 267304 | Mega Empires: The West | 2019 | yes | `Game: Civilization` eco 4, `Crowdfunding: Kickstarter` | 0 links | **eligible** | eligible | families Game: Civilization (not Mega Empires game family), **no version/contained/reimplements link** — not an edition of Civilization; title contains no edition token; no hard relationship — explicitly none |
| 373600 | Cthulhu: Death May Die – Fear of the Unknown | 2024 | yes | `Category: Dungeon Crawler` etc., **no Game:** (max_eco 2) | 0 version/contained/reimplements; 1 integration target from 253344 `integration` (537 rows 1.6% of links) but **no version/contained/reimplements link** | **eligible** | eligible | families no Game: family, integration alone (Season 2 expansion etc. are separate expansions 256972/256985 etc. but Fear is not expansion per `is_expansion False`) indicates seasonal content, not edition variant; no qualifying hard relationship — explicitly no version/expansion/reimplements link found; integration not hard per 6A |

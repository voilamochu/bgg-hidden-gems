# Second-Pass Population Methodology Review — Plan (Deferred → EXECUTED 2026-08-24, Extended 2026-08-24)

**Status:** `EXECUTED` — **deferred review now authorized and built for comparison** (see `README.md:1` and `executed_rules.md:1` in this directory, plus `data/processed/phase2-second-pass/README.md` for provenance). The current **16,627-game research population** (`data/processed/bgg_research_population.parquet`, `scripts/01_clean_population.py`) remains the **primary first-pass research record** for comparison — second-pass is `data/processed/phase2-second-pass/` (primary 16458 → closed 14786, vs base closed 14941). Requires explicit comparison before adoption. **Do not label the current population "wrong"** — treat this as a methodological refinement **enabled by newly available data** (the 9 GB BGG SQLite snapshot and its richer `game_links`/`families`/title/metadata relationships).

> **Executed 2026-08-24 via `scripts/34_second_pass_population.py` + `35_second_pass_model_comparison.py` (bounded, copy-once). This plan file is retained as historical deferred plan; see `README.md` and `executed_rules.md` for the actual executed rules, counts, and decision (estimation stable, screening noisy → n≥100 floor, not population redefinition, but dedup cleans candidate screen).**

> **Extension 2026-08-24 via `scripts/36_second_pass_audit_extension.py` + `scripts/37_second_pass_closure_and_rebuild.py` (bounded 4GB/3threads, copy-once `scratch/second-pass-audit`). Additional 100 game-entity duplicates with corroboration (97 edition_extended second-edition/anniversary/premium/heritage/decennial etc, 1 starter-set, 1 bundle, 1 reprint) => 269 total pruned (169+100) => 16358 before closure => 14698 converged in 4 iterations (1649+11 games, 946+475+3 users). New namespace `data/processed/phase2-pass2/` built with validation 0 violations, every game ≥100 and user ≥10. Downstream Phase 2/3/4 refresh remains deferred. See `game_entity_cleanup_audit.md`, `recursive_population_iterations.csv`, `population_comparison.*`, `phase2-pass2/README.md` for provenance.**

**Status (original):** `DEFERRED` — **not part of the current primary pipeline**. To be reconsidered only after the current full Phase 1–7 first pass is complete. The current **16,627-game research population** (`data/processed/bgg_research_population.parquet`, `scripts/01_clean_population.py`) remains the **primary first-pass research record**. Requires explicit comparison of the current and second-pass populations before adoption. **Do not label the current population "wrong"** — treat this as a methodological refinement **enabled by newly available data** (the 9 GB BGG SQLite snapshot and its richer `game_links`/`families`/title/metadata relationships).
